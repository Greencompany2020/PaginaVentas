...
const SideMenu = () => {
	...
	const [filteredMenu, setFilteredMenu] = useState(enlacesMenuLateral)
	...
	useEffect(() => {
        ...
        console.log(data);
				...
				const allow = new Set(
					dashboards
						.filter((d) => String(d?.Enabled).toUpperCase() === 'Y' && d?.Endpoint)
						.map((d) => d.Endpoint.trim())
				)
			...
			>
				<div className="flex flex-col h-full justify-between p-4 space-y-3">
					<div className="flex justify-between items-center">
						...
						<XIcon width={24} className=" text-white cursor-pointer" onClick={toggleVisible} />
					</div>
					...
					<div className="p-3 flex flex-row">
						...
						<button className="text-sky-500 pl-1 font-semibold" onClick={handleLogout}>
							Cerrar sesion
						</button>
					</div>
				</div>
