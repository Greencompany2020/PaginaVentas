...
export default function SelectTiendasCombo({value, onChange}) {
  return (
    <label htmlFor="mostrarTiendas" className='flex flex-col text-sm'>
        <span className='font-semibold'>{checkboxLabels.CBMOSTRARTIENDAS}</span>
        <select name="mostrarTiendas" className='h-8 border rounded-md pl-2 border-slate-400' value={value} onChange={onChange}>
        {
