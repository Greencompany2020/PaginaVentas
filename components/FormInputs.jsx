...
export const TextInput = ({ label, ...props }) => {
	...
	return (
		<label
			className={`flex flex-col font-semibold text-sm  text-gray-600 ${props.disabled && 'text-gray-100'}`}
...
/**
 * @param {string} label Valor para etiqueta de input
 * @param {FieldHookConfig<HTMLSelectElement>} props Props del elemento HTML Select. Obligatorio.
 * @returns {JSX.Element}
 * @constructor
 */
...
export const ResetViewInput = ({ label, ...props }) => {
	...
	return (
		<label className="block" htmlFor={props?.id || props?.name}>
			{label}
			<div
				className={`flex overflow-hidden h-10 border-2 rounded-md ${meta.touched && meta.error ? 'border-red-400' : 'border-slate-400'}`}
			>
				<input
					type={isVisible ? 'text' : 'password'}
...
export const CheckBoxInput = ({ label, ...props }) => {
	...
	return (
		<label
			className={`flex items-center space-x-1 font-semibold  text-gray-600 text-sm ${props.disabled && 'text-gray-100'}`}
			htmlFor={props?.id || props?.name}
		>
...
export const RadioInput = ({ label, ...props }) => {
	...
	return (
		<label
			className={`flex items-center space-x-1 font-semibold text-sm  text-gray-600 ${props.disabled && 'text-gray-100'}`}
		>
...
export const RadioImageInput = ({ label, ...props }) => {
	...
	return (
		<label
			className={`flex items-center space-x-1 font-semibold text-sm border p-1 rounded-md text-gray-600 ${isChecked ? 'border-blue-400' : 'border-slate-100'} ${props.disabled && 'opacity-40'}`}
		>
