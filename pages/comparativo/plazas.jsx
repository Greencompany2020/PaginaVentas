						...
						<table className="table-report" key={v4()} onClick={selectRow}>
							...
							<thead>
								<tr>
									...
									<th
										colSpan={5 + (date.dateRange[1] ? 3 : 0)}
									>{`Acumulado ${dateHelper.getMonthName(date.current)}`}</th>
							...
							<tbody>
								{values &&
									...
									values.map((item) => (
										<tr key={v4()}>
											<td data-type-format="text" className="priority-cell">
												{item.plaza}
											</td>
											<td className="priority-cell">
												{numberWithCommas(item['ventasMensualesActual' + dateHelper.getCurrentYear(date.current)])}
											</td>
											...
											<td data-porcent-format={isNegative(item['diferenciaMensual' + date.dateRange[0]])}>
											...
											<td className="priority-cell">
												{numberWithCommas(item['ventasAnualActual' + dateHelper.getCurrentYear(date.current)])}
											</td>
											...
											<td data-porcent-format={isNegative(item['diferenciaAnual' + date.dateRange[0]])}>
									...
									{values &&
										...
										values.map((item) => (
											<tr key={v4()}>
												...
												<td>{numberWithCommas(item['ventasMensualesActual' + date.dateRange[0]])}</td>
												<td>
													{numberWithCommas(item['presupuestoMensual' + dateHelper.getCurrentYear(date.current)])}
												</td>
												<td data-porcent-format={isNegative(item['diferenciaMensual' + date.dateRange[0]])}>
													{numberAbsComma(item['diferenciaMensual' + date.dateRange[0]])}
												</td>
										...
										{values &&
											...
											values.map((item) => (
												<tr key={v4()}>
													...
													<td>
														{numberWithCommas(item['presupuestoAnual' + dateHelper.getCurrentYear(date.current)])}
													</td>
													<td data-porcent-format={isNegative(item['diferenciaAnual' + date.dateRange[1]])}>
														{numberAbsComma(item['diferenciaAnual' + date.dateRange[1]])}
													</td>
				...
				Object.entries(data).map(([key, val]) => {
					...
					if (val && val.length > 0) {
						const Items = val.map((item) => {
							...
							return <Stats key={v4()} title={item.plaza} columns={cols} expand={true} />
						})
						return Items
					}
				})}
					...
					return (
						<React.Fragment key={v4()}>
							<div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
								...
								/>
								<Stats title={'Acumulado Anual'} expand={true} columns={acumAnualCol.flat(1)} />
