...
function ParticipacionPlaza({ config }) {
	...
	const makeOptions = (title, parts, sales, note) => ({
		...
		plugins: {
			...
			doughnutSmartLabelsPlaza: { amounts: sales, money: fmtMoney, gap: 24, lineColor: '#CBD5E1' },
							...
							{({ isSubmitting }) => (
								<Form>
									...
									<button
										...
										className={`w-full mt-2 px-4 py-2 rounded-md text-white ${loading ? 'bg-slate-400' : 'bg-sky-600 hover:bg-sky-500'}`}
									>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						<div className="rounded-xl border p-4 shadow-sm bg-white h-[420px]">
							<Doughnut
								ref={monthChartRef}
								data={chartDataMonth}
								plugins={[DoughnutSmartLabelsPlaza, CornerNote]}
								options={makeOptions(
						...
						<div className="rounded-xl border p-4 shadow-sm bg-white h-[420px]">
							<Doughnut
								...
								data={chartDataYear}
								plugins={[DoughnutSmartLabelsPlaza, CornerNote]}
								options={makeOptions(`ACUMULADO ${dateHelper.getCurrentYear(reportDate)}`, valuesYear, salesYear, nota)}
							/>
						</div>
					</div>
				)}
