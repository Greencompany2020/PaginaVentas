...
import {
  Chart as ChartJS, ArcElement, Tooltip, Legend, CategoryScale, LinearScale, BarElement, Title as ChartTitle
} from 'chart.js'
...
const DoughnutSmartLabels = {
  ...
  afterDatasetsDraw(chart, _args, opts) {
    ...
    const drawItem = (p) => {
      ...
      ctx.beginPath(); ctx.moveTo(p.anchorX, p.anchorY); ctx.lineTo(elbowX, p.labelY); ctx.lineTo(p.labelX, p.labelY); ctx.stroke()
      ctx.textAlign = p.side === 'right' ? 'left' : 'right'
      ctx.textBaseline = 'middle'
      ctx.fillText(p.text, p.labelX, p.labelY)
    }
    left.forEach(drawItem); right.forEach(drawItem)
    ctx.restore()
  }
}
...
function Marca({ config }) {
  ...
  useEffect(() => { handleSubmit(initialParams) /* eslint-disable-next-line */ }, [])
...
const makeOptions = (title, parts, sales) => ({
  ...
  plugins: {
    legend: {
      ...
      labels: {
        ...
        boxWidth: 14,
        padding: 14
        // OJO: dejamos el generateLabels por defecto de Chart.js
        // para que tome directamente data.labels (nuestros "labels")
      }
    },
    ...
    doughnutSmartLabels: { amounts: sales, money: fmtMoney, gap: 18, lineColor: '#CBD5E1', lineWidth: 1 }
  }
})
  ...
  const handleExport = async () => {
    try {
      ...
      if (imgMonth) { const id1 = wb.addImage({ base64: imgMonth, extension: 'png' }); ws.addImage(id1, { tl: { col: leftCol,  row: startRow - 1 }, ext: { width: imgW, height: imgH } }) }
      if (imgYear)  { const id2 = wb.addImage({ base64: imgYear,  extension: 'png' }); ws.addImage(id2, { tl: { col: rightCol, row: startRow - 1 }, ext: { width: imgW, height: imgH } }) }
              ...
              {({ isSubmitting }) => (
                <Form>
                  <fieldset className="space-y-2 mb-3">
                    <Input type="date" id="fecha" name="fecha" label="Fecha" placeholder={reportDate.current} disabled={loading} />
