 ...
 * @param {object} config Objeto de configuracion
 */
const addSheet = (wb, columns, rows, sheetname, format, config, footerNote = '') => {
	...
	if (config.ignoreAutoMargin == false) setAutoMargin(ws, initialDataRow, config.margin)
	// --- Nota al pie (opcional) DESPUÉS del auto-margin para no ensanchar columnas ---
	if (footerNote) {
		ws.addRow([]) // espacio
		const noteRow = ws.addRow([footerNote])
		const r = noteRow.number
		const lastCol = ws.actualColumnCount || ws.columnCount || 1
		ws.mergeCells(r, 1, r, lastCol)
		const c = ws.getCell(r, 1)
		c.alignment = { horizontal: 'center' }
		c.font = { italic: true, color: { argb: '555555' } }
	}
}
